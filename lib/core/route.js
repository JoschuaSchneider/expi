...
function Route(opts){
	...
	if(options.middleware.post && options.middleware.post.constructor !== Array)
	...
	if(options.validate && options.validate.response){
		if(typeof options.validate.response === 'function'){
			...
			this.responseValidator = options.response.request
		}
	}
...
Route.prototype.validateRequest = function(request){
	if(!this.requestValidator) return null
	return Joi.validate(request, this.requestValidator)
}

Route.prototype.validateResponse = function(response){
	if(!this.responseValidator) return null
	return Joi.validate(response, this.responseValidator)
}
